...
purify:
	...
	go-bindata -o city_bindata.go GeoLite2-City.mmdb
	rm GeoLite2-City.mmdb
	echo '' > GeoLite2-Country.mmdb
	go-bindata -o country_bindata.go GeoLite2-Country.mmdb
...
build:
	...
	env GOOS=linux go build -ldflags="-s -w" -o bin/geocoder-izi-lookup geocoder-izi-lookup.go country_bindata.go
